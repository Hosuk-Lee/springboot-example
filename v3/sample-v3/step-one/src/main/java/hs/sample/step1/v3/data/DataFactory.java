...
import hs.sample.step1.v3.data.impl.DefaultAddressDataValues;
import hs.sample.step1.v3.data.impl.DefaultContentDataValues;
import hs.sample.step1.v3.data.impl.DefaultNameDataValues;
...
public final class DataFactory {
     ...
     * <code>baseData</code> + <code>maxDaysFromData</code>. This method does not alter the time component and the time
     * is set to the time value of the base date.
     *
    ...
    private void validateMinMaxParams(final int minLength, final int maxLength) {
        ...
        if (maxLength < minLength) {
            throw new IllegalArgumentException(
                    String.format("Minimum length must be less than maximum length (min=%d, max=%d)", minLength,
                            maxLength));
        }
    }
    ...
    /**
     * Returns a valid word based on the length range passed in. The length will always be between the min and max
     * length range inclusive.
     *
    ...
    /**
     * Call randomize with a seed value to reset the random number generator. By using the same seed over different
     * tests, you will should get the same results out for the same data generation calls.
     *
